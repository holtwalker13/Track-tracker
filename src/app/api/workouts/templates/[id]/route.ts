...
export async function PATCH(
  ...
  try {
    ...
    for (const [i, input] of exercises.entries()) {
      ...
      const sets = Math.min(20, Math.max(1, Number(input.defaultSets) || 3));
      const reps = Math.min(50, Math.max(1, Number(input.defaultReps) || 5));
      ...
      if (prev) {
        ...
        await tx.workoutTemplateExercise.update({
          where: { id: prev.id },
          data: { defaultSets: sets, defaultReps: reps, notes, sortOrder: i, activityId },
        });
      } else {
